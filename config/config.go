...
func LoadAppConfig(configPath string) (*AppConfig, error) {
	...
	if v.ConfigFileUsed() == "" {
		...
		log.Structured(&pterm.Info, "Client config written to ", log.Fields{
			log.ConfigPath: writePath,
		})
	}
	...
	if v.ConfigFileUsed() == "" {
		...
		log.Structured(&pterm.Info, "Server config written to ", log.Fields{
			log.ConfigPath: writePath,
		})
	}
...
func LoadServerConfig(configPath string) (*ServerConfig, error) {
	...
	log.Structured(&pterm.Info, "TLS cert paths ", log.Fields{
		log.ServerCertificatePath: cfg.ServerCertificatePath,
		log.ServerKeyPath:         cfg.ServerKeyPath,
		log.CredentialPath:        cfg.CredentialsFile,
	})
...
func initViper(configPath, defaultDir, defaultName, defaultType, envPrefix string) (*viper.Viper, error) {
	...
	if err := v.ReadInConfig(); err != nil {
		...
		if !notFound {
			log.Structured(&pterm.Error, "Config file not found at ", log.Fields{
